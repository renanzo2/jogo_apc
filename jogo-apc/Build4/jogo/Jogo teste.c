...
void ordenarRecordes() {
    ...
    if(arquivo != NULL) {
        while(fscanf(arquivo, "%s %d %d", recordes[numRecordes].nome, 
              &recordes[numRecordes].pontos, &recordes[numRecordes].nivel) != EOF 
              && numRecordes < MAX_RECORDES) {
            numRecordes++;
        }
        fclose(arquivo);
    }
    ...
    // Reescreve arquivo ordenado
    arquivo = fopen("recordes.txt", "w");
    if(arquivo != NULL) {
        for(int i = 0; i < (numRecordes < MAX_RECORDES ? numRecordes : MAX_RECORDES); i++) {
            fprintf(arquivo, "%s %d %d\n", recordes[i].nome, recordes[i].pontos, recordes[i].nivel);
        }
        fclose(arquivo);
    }
}
